...
?>	
			<div class="msg_thumb_wrapper" <?php if($j>1) echo " style='display:none;'"; ?>>
	<?php
				for($i=$j;$i<$total;$i++){
	?>
				<a href="#"><img src="IMAGES/portfolios/<?php echo $dossier; ?>/thumbs/<?php echo $arrayPortfolio[$i]; ?>" alt="IMAGES/portfolios/<?php echo $dossier; ?>/<?php echo $arrayPortfolio[$i]; ?>"/></a>
<?php
				}
				$nb--;
?>
...
?>
		...
		<p id="nameDir"><?php echo $dossier; ?></p>
		<div id="namePic"></div>
	</div>
...
$(function() {
						...
						if($currentImage.length){
							$currentImage.fadeOut(function(){
								...
								$('<img />').load(function(){
									...
									var nom=$image.attr('src');
									var nom2=nom.replace('IMAGES/portfolios/<?php echo $dossier; ?>/','');
									var extension=nom2.substring(nom2.lastIndexOf("."));
									nom2=nom2.replace(extension,'');
									$('#namePic').html(nom2);
								}).attr('src',source);
							});
						}
				...
				*/
				...
				/**
				* resize the image to fit in the container (440 x 440)
				*/
				function resize($image){
					var theImage 	= new Image();
					theImage.src 	= $image.attr("src");
					var imgwidth 	= theImage.width*2;
					var imgheight 	= theImage.height*2;
