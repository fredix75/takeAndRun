...
//Méthode qui permet de remettre les valeurs à l'intérieur d'un div à null quelque soit le composant.
function unsetDiv(divToUnset){	
	var inputText="#"+divToUnset+" :text";
	var inputRadio="#"+divToUnset+" :radio";
	var inputCheckbox="#"+divToUnset+" :checkbox";
	var textareaU="#"+divToUnset+" textarea";
	var inputSelect="#"+divToUnset+" option[value='-1']";

	$(inputText).each(function(){ this.value = ""; });
	$(inputRadio).each(function(){ this.checked = false; });
	$(inputCheckbox).each(function(){ this.checked = false; });
	$(textareaU).each(function(){ this.value = ""; });
	$(inputSelect).attr("selected", "selected");
}

function afficheCacheCB(idCheckBox, idDiv){
	var idD="#"+idDiv+"";
	var idCB="#"+idCheckBox+"";
	var value=$(idCB).is(':checked');
...
function cacheAfficheCB(idCheckBox, idDiv){
	var idD="#"+idDiv+"";
	var idCB="#"+idCheckBox+"";
	var value=$(idCB).is(':checked');
...
function afficheCacheSelect(idSelect,valToaffiche,idToAffiche,idToCache){
	var idDIV="#"+idSelect+"";
	var value=$(idDIV).val();
	if(value==valToaffiche){
		affiche(idToAffiche);
	}else{
		unsetDiv(idToCache);
		cache(idToCache);
...
function afficheCacheSelectAutre(idSelect){
	var idDIV="#"+idSelect+"";
	var value=$(idDIV).val();
	...
	if(value=='2'){		
		...
		var inputSelect="#genre option[value='-1']";
		$(inputSelect).attr("selected", "selected");
		$("#genre").hide('slow');		
	}	
}
...
function affichePic(dir,pic,lien){
	var num=pic.indexOf('/');
	var nom=pic.substring(0,num);
	if(num!=0){
		$('#imagePres h2').html(nom);
		$('#imagePres img').attr('src',dir+pic);
		$('#imagePres .link').attr('href','index.page.php?link=dossier&cat=slide&s='+lien);		
	}else{
		alert('blemPro');
	}
}

function selPeriode(id){
	var idDIV="#"+id+"";
	var value=$(idDIV).val();
	if(value<=2015 && value>=2001){
		var periode=$("#VIDEO_PERIODE option[value='2001-2015']");
		periode.attr("selected", "selected");
	}else if(value<=2000 && value>=1986){
		var periode=$("#VIDEO_PERIODE option[value='1986-2000']");
		periode.attr("selected", "selected");
	}else if(value<=1985 && value>=1971){
		var periode=$("#VIDEO_PERIODE option[value='1971-1985']");
		periode.attr("selected", "selected");
	}else if(value<=1970 && value>=1956){
		var periode=$("#VIDEO_PERIODE option[value='1956-1970']");
		periode.attr("selected", "selected");
	}else if(value<=1955 && value>=1940){
		var periode=$("#VIDEO_PERIODE option[value='1940-1955']");
		periode.attr("selected", "selected");
	}else if(value<=1939){
		var periode=$("#VIDEO_PERIODE option[value='<1939']");
		periode.attr("selected", "selected");
	}
}
